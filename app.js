...
let selected="lead", playing=false, schedulerStep=0, playheadStep=0, nextTime=0, timer=null, audio=null, patchNo=1, percLayerCount=1;
let state={name:"STATIC PRESSURE",bpm:118,root:6,scale:"minor",fx:{filter:true,delay:true,chorus:false,crush:false},tracks:{}};
function blankTrack(t){return {muted:false,solo:false,volume:t.type==="drum"?.72:.65,tone:t.id==="bass"?.28:.55,pitch:t.id==="kick"?.32:.55,decay:t.type==="drum"?.28:.6,steps:Array.from({length:16},()=>t.type==="melodic"?{on:false,midi:60,vel:.7}:{on:false,vel:.75})}}
...
function previewSelected(){
 if(!audio)return;
 const meta=TRACKS.find(t=>t.id===selected),now=audio.ctx.currentTime+.01;
 if(meta.type==="melodic"){
  const step=state.tracks[selected].steps.find(s=>s.on)||{midi:selected==="bass"?43:60,vel:.8};
  synth(selected,step.midi,now,.85)
 }else drum(selected,now)
}
function renderSoundControls(){
 const tr=state.tracks[selected],meta=TRACKS.find(t=>t.id===selected);
 const firstLabel=meta.type==="melodic"?"TONE":"PITCH";
 const firstValue=meta.type==="melodic"?tr.tone:tr.pitch;
 $("#soundControls").innerHTML=`
  <div class="sound-control"><label>${firstLabel} <input id="toneCtl" type="range" min="0" max="1" step=".01" value="${firstValue}"></label></div>
  <div class="sound-control"><label>DECAY <input id="decayCtl" type="range" min=".05" max="1.5" step=".01" value="${tr.decay}"></label></div>
  <button id="auditionBtn" style="width:100%;margin-top:8px">AUDITION ${meta.name}</button>`;
 $("#toneCtl").oninput=e=>{if(meta.type==="melodic")tr.tone=+e.target.value;else tr.pitch=+e.target.value;previewSelected()};
 $("#decayCtl").oninput=e=>{tr.decay=+e.target.value;previewSelected()};
 $("#auditionBtn").onclick=()=>{initAudio();audio.ctx.resume();previewSelected()}
}
...
function synth(id,midi,time,vel=.7){
 if(!audible(id))return;initAudio();
 const tr=state.tracks[id],o=audio.ctx.createOscillator(),voiceFilter=audio.ctx.createBiquadFilter(),g=audio.ctx.createGain();
 o.type=id==="bass"?"square":tr.tone>.66?"sawtooth":tr.tone>.33?"triangle":"sine";
 o.frequency.value=freq(midi);
 voiceFilter.type="lowpass";
 voiceFilter.frequency.value=350+tr.tone*7800;
 voiceFilter.Q.value=.4+tr.tone*5;
 g.gain.setValueAtTime(.0001,time);g.gain.exponentialRampToValueAtTime(Math.max(.001,tr.volume*vel*.18),time+.01);g.gain.exponentialRampToValueAtTime(.0001,time+tr.decay);
 o.connect(voiceFilter);voiceFilter.connect(g);g.connect(audio.filter);o.start(time);o.stop(time+tr.decay+.05)
}
function drum(id,time){
 if(!audible(id))return;initAudio();const tr=state.tracks[id],ctx=audio.ctx,g=ctx.createGain(),pitch=tr.pitch??.5;
 if(id==="kick"){
  const o=ctx.createOscillator();o.frequency.setValueAtTime(95+pitch*115,time);o.frequency.exponentialRampToValueAtTime(32+pitch*32,time+.16);g.gain.setValueAtTime(tr.volume*.65,time);g.gain.exponentialRampToValueAtTime(.001,time+tr.decay);o.connect(g);g.connect(audio.master);o.start(time);o.stop(time+tr.decay)
 }else{
  const len=Math.floor(ctx.sampleRate*(id==="hat"?.05:tr.decay)),b=ctx.createBuffer(1,len,ctx.sampleRate),d=b.getChannelData(0);for(let i=0;i<len;i++)d[i]=Math.random()*2-1;
  const source=ctx.createBufferSource(),f=ctx.createBiquadFilter();source.buffer=b;
  const isSnare=id==="snare",isHat=id==="hat";
  f.type=isSnare?"bandpass":"highpass";
  f.frequency.value=isSnare?700+pitch*3200:isHat?3500+pitch*7500:900+pitch*6500;
  f.Q.value=.6+pitch*3;
  g.gain.setValueAtTime(tr.volume*(isHat?.16:.32),time);g.gain.exponentialRampToValueAtTime(.001,time+Math.max(.04,tr.decay));source.connect(f);f.connect(g);g.connect(audio.master);source.start(time)
 }
}
...
function addPercLayer(){
 percLayerCount+=1;
 const id=`perc${percLayerCount}`;
 const colors=["#72efb3","#ff8fd8","#8fa7ff","#f2ff8f","#7ff3ff"];
 const track={id,name:`PERC ${percLayerCount}`,type:"drum",color:colors[(percLayerCount-2)%colors.length],desc:"Custom percussion layer"};
 TRACKS.push(track);
 state.tracks[id]=blankTrack(track);
 state.tracks[id].pitch=.35+Math.random()*.5;
 state.tracks[id].decay=.12+Math.random()*.35;
 state.tracks[id].steps.forEach((step,i)=>step.on=Math.random()<.16&&(i%2===1||Math.random()<.3));
 selected=id;
 renderAll();
 toast(`${track.name} ADDED`)
}
...
$("#trackVolume").oninput=e=>{state.tracks[selected].volume=+e.target.value;previewSelected()};$("#clearTrackBtn").onclick=()=>{state.tracks[selected].steps.forEach(s=>s.on=false);renderAll()};$("#shiftLeftBtn").onclick=()=>shift(-1);$("#shiftRightBtn").onclick=()=>shift(1);
...
$("#addPercBtn").onclick=addPercLayer;
