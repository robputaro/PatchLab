...
let patchCount=1,playing=false,currentStep=0,nextNoteTime=0,timer=null,audio=null,recording=false,recorder=null,chunks=[],selectedJack=null,audioUnlocked=false;
let patch={
...
function buildKeyboard(){const ms=[60,61,62,63,64,65,66,67,68,69,70,71,72];$("#keyboard").innerHTML=ms.map((m,i)=>`<button class="key ${NOTES[m%12].includes("#")?"black":""}" data-midi="${m}">${KEYMAP[i].toUpperCase()}</button>`).join("");$$(".key").forEach(k=>{k.onpointerdown=async()=>{if(!audioUnlocked){await unlockAudio()}k.classList.add("down");triggerLead(+k.dataset.midi,audio?.ctx?.currentTime||0,.8)};k.onpointerup=()=>k.classList.remove("down")})}
...
async function unlockAudio(){
  ...
  try{
    ...
    source.buffer=buffer;source.connect(audio.ctx.destination);source.start(0);
    audioUnlocked=audio.ctx.state==="running";
    if(audioUnlocked){$("#audioGate").classList.add("hidden");toast("AUDIO ENABLED");}
    else toast("TAP AGAIN TO ENABLE AUDIO");
  }catch(err){console.error(err);toast("AUDIO COULD NOT START");}
}
function initAudio(){
  ...
  filter.connect(fxIn);fxIn.connect(dry);fxIn.connect(delay);delay.connect(feedback);feedback.connect(delay);delay.connect(wet);fxIn.connect(chorus);chorus.connect(wet);dry.connect(master);wet.connect(master);direct.connect(master);
  master.connect(compressor);compressor.connect(analyser);compressor.connect(mediaDest);analyser.connect(ctx.destination);
  const lfo=ctx.createOscillator(),lfoGain=ctx.createGain();lfo.connect(lfoGain);lfoGain.connect(filter.frequency);lfo.start();
...
async function start(){initAudio();if(audio.ctx.state!=="running")await audio.ctx.resume();audioUnlocked=audio.ctx.state==="running";if(!audioUnlocked){$("#audioGate").classList.remove("hidden");return}if(playing)return;playing=true;currentStep=0;nextNoteTime=audio.ctx.currentTime+.05;timer=setInterval(scheduler,25);$("#playBtn").textContent="■";$("#nowPlaying").textContent="PLAYING";ensureDrone();toast("PERFORMANCE STARTED")}
...
$("#advancedToggle").onclick=()=>{
  const blocks=$$(".advanced-patching"),opening=!blocks[0].classList.contains("open");
  blocks.forEach(b=>b.classList.toggle("open",opening));
  $("#advancedToggle").textContent=opening?"HIDE ADVANCED PATCHING":"SHOW ADVANCED PATCHING";
  if(opening)setTimeout(drawCables,80);
};
["pointerdown","touchstart"].forEach(evt=>document.addEventListener(evt,()=>{if(audio&&audio.ctx.state==="suspended")audio.ctx.resume()},{passive:true}));
