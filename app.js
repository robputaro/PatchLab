...
let selected="lead", playing=false, currentStep=0, nextTime=0, timer=null, audio=null, patchNo=1;
...
function renderEditor(){
 ...
 if(meta.type==="melodic"){
  $("#trackEditor").className="track-editor melodic-grid";$("#trackEditor").innerHTML=tr.steps.map((s,i)=>`<div class="melodic-step ${s.on?"active":""} ${i===currentStep&&playing?"current":""}" data-step="${i}"><span class="note">${noteName(s.midi)}</span><div class="bar" style="height:${Math.round(s.vel*88)}%"></div></div>`).join("");
  ...
  $("#trackEditor").className="track-editor drum-grid";$("#trackEditor").innerHTML=tr.steps.map((s,i)=>`<div class="drum-step ${s.on?"active":""} ${i===currentStep&&playing?"current":""}" data-step="${i}"></div>`).join("");$$(".drum-step").forEach(el=>el.onclick=()=>{tr.steps[+el.dataset.step].on=!tr.steps[+el.dataset.step].on;renderAll()})
 }
 renderSoundControls()
}
...
function schedule(step,time){TRACKS.forEach(t=>{const s=state.tracks[t.id].steps[step];if(!s.on)return;t.type==="melodic"?synth(t.id,s.midi,time,s.vel):drum(t.id,time)});setTimeout(()=>{currentStep=step;renderEditor();renderArrangement()},Math.max(0,(time-audio.ctx.currentTime)*1000))}
function scheduler(){while(nextTime<audio.ctx.currentTime+.1){schedule(currentStep,nextTime);nextTime+=60/state.bpm/4;currentStep=(currentStep+1)%16}}
function play(){initAudio();audio.ctx.resume();if(playing)return;playing=true;currentStep=0;nextTime=audio.ctx.currentTime+.05;timer=setInterval(scheduler,25);$("#playState").textContent="PLAYING";$("#playBtn").textContent="❚❚"}
function stop(){playing=false;clearInterval(timer);timer=null;currentStep=0;$("#playState").textContent="STOPPED";$("#playBtn").textContent="▶";renderAll()}
