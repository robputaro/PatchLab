...
- Add Perc Layer now creates a genuine independent percussion track with its own steps, mute, solo, volume, pitch, and decay.
- Track Sound controls now directly change each voice.
- Melodic tone now changes waveform and per-voice filter brightness.
- Drum pitch now changes the actual kick/noise-filter frequency.
- Moving a sound control auditions the selected instrument immediately after audio has been enabled.
